...
@dag(
     dag_id="delta_trigger_dag",
     schedule=DeltaTriggerTimetable(pendulum.duration(days=10)), # this will set the start date to 10 days from the time the DAG is unpaused
     start_date=pendulum.datetime(year=2026, month=6, day=20, tz="America/New_York"),
     is_paused_upon_creation=True)
def delta_trigger_dag():
    @task.python
...
delta_trigger_dag_instance = delta_trigger_dag()