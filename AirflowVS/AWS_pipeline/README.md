...
Airflow runs self-hosted (outside AWS, via Docker Compose in this repo) and orchestrates the pipeline entirely through AWS API calls (boto3) and the Databricks SDK — it never hosts data itself, only triggers and waits on remote jobs.
...
![Airflow DAG run](docs/airlfow.png)
